...
class GovernmentEmailSender:
    def __init__(self):
        ...
        self.cc_emails = self._validate_cc_emails(CC_EMAILS) if EMAIL_DISTRIBUTION_CONFIG.get('use_cc', False) else []
        
        # Validate and set BCC emails
        self.bcc_emails = self._validate_bcc_emails(BCC_EMAILS) if EMAIL_DISTRIBUTION_CONFIG.get('use_bcc', False) else []
        ...
        print(f"   CC recipients: {len(self.cc_emails)} (enabled: {self.email_distribution_config.get('use_cc', False)})")
        print(f"   BCC recipients: {len(self.bcc_emails)} (enabled: {self.email_distribution_config.get('use_bcc', False)})")
    ...
    def _get_email_distribution_list(self):
        ...
        if distribution_config.get('use_cc', False) and self.cc_emails:
            ...
            if cc_handling == 'error':
                print("⚠️  CC emails enabled but none configured - continuing with primary recipients only")
        ...
        if distribution_config.get('use_bcc', False) and self.bcc_emails:
            ...
            if bcc_handling == 'error':
                print("⚠️  BCC emails enabled but none configured - continuing with primary recipients only")
        ...
        if distribution_config.get('log_distribution', False):
            ...
            print(f"   Total: {len(all_recipients) + len(cc_list) + len(bcc_list)} recipients")
    ...
    def send_email(self, service, template):
        ...
        try:
            ...
            try:
                text = msg.as_string()
                server.sendmail(service['email'], distribution['to'] + distribution['cc'] + distribution['bcc'], text)
                server.quit()
